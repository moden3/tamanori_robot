...
int motor_flag = 0;  // モータを動かすときに立てるフラグ
...
void motor_pid(double speedref[3]){
    ...
    for (int i = 0; i < 3;i++){
        ...
        if(-3.0<speedref[i] && speedref[i]<3.0){
            speedref_ = 0.0;
        }

        // 単位換算
        en_speed[i] = delta_stock[i] * 2 * PI / 360 / (double)SUMPLING_TIME_US * 1000000.0 * 2/5;

        double dev = speedref_ - en_speed[i];
        sum[i] += ((dev - olddev[i]) * (double)SUMPLING_TIME_US / 2000000.0);
        double delta = (dev - olddev[i]) / (double)SUMPLING_TIME_US * 1000000.0;
